 ...
 */
public class NCharGramTokenizer extends CharacterDelimitedTokenizer {
	...
	public Enumeration listOptions() {
		...
		result.addElement(new Option(
				"\tThe max size of the Ngram (default = 6).", "max", 1,
				"-max <int>"));

		result.addElement(new Option(
				"\tThe min size of the Ngram (default = 3).", "min", 1,
				"-min <int>"));

		result.addElement(new Option("\tDon't use words as features.",
				"exword", 0, "-exword"));
	...
	public String nextElement() {
		...
		if (m_N == 0) {
			...
			retValue = m_SplitString[m_CurrentPosition].substring(
					m_CurrentChar, m_CurrentChar + m_N);

			m_CurrentChar++;

			if (m_CurrentChar + m_N - 1 == m_SplitString[m_CurrentPosition]
					.length()) {
				m_CurrentChar = 0;
				m_CurrentPosition++;
			}
	...
	/**
	 * filters out empty strings in m_SplitString and replaces m_SplitString
